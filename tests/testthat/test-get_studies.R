...
#     skip_if_testing_is_fast()
#     with_mocked_bindings(readline = function(...) 'y', my_studies <- get_studies(interactive = TRUE))
#     expect_is(my_studies, 'studies')
#   })
# })
#
...
#     skip_if_testing_is_fast()
#     with_mocked_bindings(readline = function(...) 'n', my_studies <- get_studies(interactive = TRUE))
...
test_that("get_studies_by_efo_uri: status code is not 200", {
  ...
  local_mocked_bindings(gc_get = function(...) bad_response)
  expect_identical(get_studies(efo_uri = efo_uris), studies())
})
